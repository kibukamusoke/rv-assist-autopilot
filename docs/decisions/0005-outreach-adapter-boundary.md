...
The hackathon deployment uses `MockOutreachAdapter`. It produces deterministic, idempotent synthetic deliveries and never contacts a real person. Successful technician delivery is required before scheduling a response deadline. Failed technician delivery advances to the next candidate; exhaustion escalates to a human. Failed customer delivery also escalates.

## Consequences

- Judges can see action evidence rather than only a contact-state label.
